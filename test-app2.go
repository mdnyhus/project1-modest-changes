...
func main() {
	...
	shapeHash, _, inkRemaining, err := canvas.AddShape(1, blockartlib.PATH, "M 20 20 H 5 V 5 h -5 Z", "red", "blue")
	fmt.Println(shapeHash)
	fmt.Println(inkRemaining)
	fmt.Println(err)
	shapeHash, _, inkRemaining, err = canvas.AddShape(1, blockartlib.PATH, "M 25 25 h 10 v 10 h -5 Z", "green", "blue")
	fmt.Println(shapeHash)
	fmt.Println(inkRemaining)
	fmt.Println(err)
	shapeHash, _, inkRemaining, err = canvas.AddShape(1, blockartlib.PATH, "M 40 40 h 5 v 5 h -5 Z", "yellow", "blue")
	fmt.Println(shapeHash)
	fmt.Println(inkRemaining)
	fmt.Println(err)
